...
def test_calculate_cgpa_with_courses():
    ...
    # Create DetailedCourseResult objects
    ...
    total_points = (
        (5 * 3) + (4 * 4) + (5 * 2) + (3 * 3) +  # Level 100, Harmattan and Rain
        (4 * 3) + (5 * 3) + (2 * 4) + (3 * 2)    # Level 200, Harmattan and Rain
    )
    ...
    # Verify that the calculated CGPA matches the expected CGPA
    print(
    ...
    assert len(semester_cgpas) == 4  # Expecting 4 CGPAs: 2 semesters for level 100 and 2 semesters for level 200
